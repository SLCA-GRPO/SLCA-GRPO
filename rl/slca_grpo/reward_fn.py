"""
HierR reward function used by SLCA-GRPO.

Score composition:
- Process reward = format + tool-name F1 + param-key F1 + param-value F1 + parallel-match
- Summary reward = LLM-judge response quality (given the tool results, how good is the final reply)
- score = SLCA_WEIGHT_PROCESS * process + SLCA_WEIGHT_RESPQ * summary

`compute_score` is the async entry point verl calls via `custom_reward_function.path`.

Environment variables:
- LLM_JUDGE_BASE_URL, LLM_JUDGE_MODEL, LLM_JUDGE_ENABLED: LLM-judge endpoint / toggle
- REWARD_LOG_DIR: directory where per-process reward logs are written
- SLCA_WEIGHT_FORMAT / NAME / KEY / VALUE / PARALLEL: process sub-weights
- SLCA_WEIGHT_PROCESS / SLCA_WEIGHT_RESPQ: top-level process vs. summary weights
"""
...
_log_formatter = logging.Formatter(
    '[%(asctime)s][HierR][%(levelname)s] %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S'
)
...
_log_file_path = _os.path.join(_log_dir, f'hierr_reward_{_log_timestamp}_pid{_os.getpid()}.log')
...
# Rating-to-score map for response quality
...
# Accept both <think> and <thinking> tags.
...
def _loose_value_equal(pred_val: Any, gold_val: Any) -> bool:
    """Loose equality: strings (trim+lower), numbers, booleans, sorted JSON for list/dict."""
    # 1. Both None / empty
    ...
    # 4. Boolean compare: true == "true" == True
    ...
    # 5. Lists/dicts: compare via sorted JSON dump
...
def _extract_final_response(solution_str: str) -> str:
    """Pull the final assistant reply (after the last tool_response / tool_call) out of solution_str."""
    if not solution_str:
        return ""
...
def _extract_tool_responses(solution_str: str) -> str:
    """Collect only <tool_response> blocks (no tool_call content).

    Rationale: tool-call correctness is already covered by the rule-based process
    reward, so the LLM judge only needs the tool results when scoring the reply.
    """
    if not solution_str:
        return ""
    responses = []

    # Only keep <tool_response>...</tool_response> content.
    tool_responses = TOOL_RESPONSE_PATTERN.findall(solution_str)

    for i, response_content in enumerate(tool_responses):
        responses.append(f"[Tool Response {i+1}]")
...
def _check_tool_call_format(
    ...
    """Check <tool_call> format and return the parsed calls grouped by block."""
    tool_calls: List[ToolCall] = []
    tool_calls_by_block: List[List[ToolCall]] = []
    ...
    for match in matches:
        ...
        if objs is None:
            if first_error_type is None:
                first_error_type = "json_error"
                first_error_message = f"JSON must be object or array"
            continue
...
def _compute_format_score(text: str, allowed_tools: List[str], tool_schemas=None) -> Tuple[float, FormatCheckResult]:
    """Compute the format score in [0, 1].

    <think> content is stripped first so any tool_call written inside a chain of
    thought is not counted as a real call.
    """
    # Strip <think> blocks before parsing tool calls.
...
def _compute_detailed_match_scores(
    ...
    """Compute name F1 plus per-arg key/value scores via greedy matching.

    Returns:
        name_match_score: tool-name F1 in [0, 1]
        key_match_score: argument-key match score in [0, 1]
        value_match_score: argument-value match score in [0, 1]
    """
    ...
    # 2. Argument match (only for gold calls that actually have arguments)
    ...
    if not gold_with_args:
        # No gold calls have arguments -> key/value trivially perfect.
        return name_match_score, 1.0, 1.0

    if not pred_calls:
        return name_match_score, 0.0, 0.0

    # Greedy: for each gold call, pick the best unused pred with matching name.
    total_key_score = 0.0
    ...
    for gold in gold_with_args:
        ...
        for pred_idx, pred in enumerate(pred_calls):
            ...
            # Value score based on intersected keys
            if not gold_keys:
                value_score = 1.0
            else:
...
def _compute_parallel_score(
    ...
    """Binary match on first-step parallel count.

    Returns (1.0 if equal else 0.0, first_gold_count, first_pred_count).
    """
...
2. **Information Completeness**: Does the response address all parts of the user's question based on the tool results?
...
async def _compute_response_quality_score(
    ...
    """Score the final reply.

    If there are no tool_response blocks, return 0 immediately (nothing to judge
    against). Otherwise call the LLM judge and map the rating to [0, 1].
    """
    # No tool_response: short-circuit to 0 (checked before LLM_JUDGE_ENABLED).
    if not tool_responses or len(tool_responses.strip()) == 0:
        logger.info("[ResponseQuality] no tool_response; summary_score = 0")
        return 0.0, "no_tool_response", None, False, False

    if not LLM_JUDGE_ENABLED:
        return 0.0, None, None, False, False

    if not final_response or len(final_response.strip()) < 10:
        logger.info("[ResponseQuality] final response missing/too short; low score")
...
def _normalize_gold_tool_calls_nested(gold_tool_calls: Any) -> Tuple[List[List[Dict[str, Any]]], List[Dict[str, Any]]]:
    """Return (nested by step, flattened) lists of gold tool calls."""
...
def _build_error_result(score: float, error_type: str) -> Dict[str, Any]:
    """Zero-filled reward dict for early-exit error paths."""
    # Read weights (kept for signature parity with normal path).
...
async def compute_score(
    ...
    **kwargs,
    ...
    """Compute the HierR reward for one rollout.

    Pipeline:
      1. Rule-based process score: format + name F1 + key F1 + value F1 + parallel.
      2. LLM-judge summary score on the final reply, using only <tool_response> blocks.
      3. score = SLCA_WEIGHT_PROCESS * process + SLCA_WEIGHT_RESPQ * summary.

    Args:
        data_source: data source identifier
        solution_str: full model completion
        ground_truth: dict with allowed_tools / gold_tool_calls / question_content
        extra_info: optional extra context

    Returns:
        Dict with `score` plus per-metric fields.
    """
    logger.info(f"[ComputeScore] v8.2_new_prompt start, data_source={data_source}")
    ...
    # Gold expects a call but model produced none -> penalty.
    if gold_flat and format_result.open_count == 0:
        first_gold = len(gold_nested[0]) if gold_nested else 0
        # Penalty path: still report normalized score.
    ...
    # Extract tool responses (only <tool_response>) and the final reply.
    tool_responses = _extract_tool_responses(solution_str)
    final_response = _extract_final_response(solution_str)

    logger.info(f"[ComputeScore] tool_responses: {tool_responses[:200] if tool_responses else '(empty)'}...")
    ...
    response_quality_score, rating, _, judge_failed, parse_failed = await _compute_response_quality_score(
        question_content, tool_responses, final_response
    )

    # Flag the "no tool_response -> 0" path.
    no_tool_response = 1 if rating == "no_tool_response" else 0

    logger.info(f"[ComputeScore] response_quality_score={response_quality_score:.3f}, rating={rating}, no_tool_response={no_tool_response}")
    logger.info(f"[ComputeScore] name={name_match_score:.3f}, key={key_match_score:.3f}, value={value_match_score:.3f}")

    # ========== v8.2 score composition ==========
    # 1. process_score: weighted combination of sub-scores, in [0, 1].
    process_score = (
    ...
    # 2. summary_score = response_quality in [0, 1].
    summary_score = response_quality_score

    # 3. weighted_score (unnormalized; used as the RL reward).
    weighted_score = w_process * process_score + w_respq * summary_score

    # 4. weighted_score_norm (normalized; used for success@ thresholds).
    ...
    logger.info(f"[ComputeScore] * v8.2NP weighted: process={process_score:.4f}, summary={summary_score:.4f}")
    ...
    # Compatibility aliases
    tool_match_score = name_match_score  # alias for name F1
    arg_match_score = (key_match_score + value_match_score) / 2.0  # legacy average
    ...
    return {
        ...
        # Segment scores (process is sub-weighted, summary is raw).
        ...
        "reward/tool_call/gold_tool_count": len(gold_flat),
        # Legacy field, equal to weighted_score_norm.
        "reward/tool_call/total_score": weighted_score_norm,
    }
...
def test_reward_function():
    """Smoke tests for the HierR reward function."""
    def print_result(name, result):
        ...
        print(f"  no_tool_response: {result.get('reward/tool_call/no_tool_response')}")
    ...
    print("=" * 60)
    print("Test: response-quality-only summary reward")
    print("=" * 60)
    print(f"Weights:")
    ...
    # Test 1: full match with a tool_response present.
    s1 = '''<tool_call>
...
Seattle is sunny today with a temperature of 25 degrees.'''
    g1 = {
        "allowed_tools": ["get_weather"],
        "gold_tool_calls": [[{"name": "get_weather", "arguments": {"city": "Seattle"}}]],
        "question_content": "What is the weather in Seattle?"
    }
    print_result("Test 1: full match (tool_response present)", compute_score_sync("test", s1, g1))

    # Test 2: no tool_response -> summary_score must be 0.
    s2 = '''<tool_call>
{"name": "get_weather", "arguments": {"city": "Seattle"}}
</tool_call>
Seattle is sunny today with a temperature of 25 degrees.'''
    ...
    print_result("Test 2: missing tool_response (summary_score should be 0)", compute_score_sync("test", s2, g2))

    # Test 3: parallel calls with tool_response blocks.
    s3 = '''<tool_call>
[{"name": "get_weather", "arguments": {"city": "Seattle"}}, {"name": "get_weather", "arguments": {"city": "Boston"}}]
</tool_call>
<tool_response>
{"temperature": 25}
</tool_response>
...
Seattle 25 degrees, Boston 28 degrees.'''
    g3 = {
        "allowed_tools": ["get_weather"],
        "gold_tool_calls": [[{"name": "get_weather", "arguments": {"city": "Seattle"}}, {"name": "get_weather", "arguments": {"city": "Boston"}}]],
        "question_content": "weather"
    }
    print_result("Test 3: parallel calls with tool_response", compute_score_sync("test", s3, g3))
