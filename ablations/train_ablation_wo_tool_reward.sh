#!/usr/bin/env bash
#
# Ablation (iii): w/o Tool-Call Reward
# Removes the tool-call process reward; only the LLM judge score remains.
#
# ================================================================================
...
#   summary_score = response_quality(1.0)
#
# Ablated variant:
#   process_score = 0 (every tool-call reward weight is set to 0)
#   summary_score = response_quality (1.0); only the judge score remains
#
# Key change:
#   SLCA_WEIGHT_FORMAT=0.0
#   SLCA_WEIGHT_NAME=0.0
#   SLCA_WEIGHT_KEY=0.0
#   SLCA_WEIGHT_VALUE=0.0
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/ablations/wo_tool_reward}"

# Reward function
REWARD_FN_PATH="${REWARD_FN_PATH:-${SCRIPT_DIR}/reward_fn_hierr_v82.py}"

# ============================================================================
# KEY CHANGE: zero out every tool-call reward weight
# ============================================================================
export SLCA_WEIGHT_FORMAT="0.0"     # format reward removed
export SLCA_WEIGHT_NAME="0.0"       # tool-name reward removed
export SLCA_WEIGHT_KEY="0.0"        # argument-key reward removed
export SLCA_WEIGHT_VALUE="0.0"      # argument-value reward removed
export SLCA_WEIGHT_PARALLEL="0.0"   # parallel-call reward removed
export SLCA_WEIGHT_RESPQ="1.0"      # only response_quality (judge score) remains
...
echo "==========================================="
echo "Ablation (iii): w/o Tool-Call Reward"
echo "==========================================="
echo ""
echo "Key changes:"
echo "  - all tool-call process rewards removed"
echo "  - only the LLM judge response_quality score remains"
echo ""
echo "Weights:"
echo "  Process segment (all set to 0):"
...
echo ""
echo "LLM judge: ${LLM_JUDGE_ENABLED} (must be enabled)"
echo "==========================================="
...
if [ "${LLM_JUDGE_ENABLED}" = "true" ]; then
    ...
    echo "Warning: LLM_JUDGE_ENABLED=false, but this ablation needs the judge score."
    echo "   Set LLM_JUDGE_ENABLED=true."
fi
...
EXPERIMENT_NAME="ablation_wo_tool_reward"
N_GPUS_PER_NODE=8
NNODES="${NNODES:-4}"
...
echo ""
echo "==========================================="
echo "Ablation (iii) w/o Tool-Call Reward finished."
echo "output dir : ${OUTPUT_DIR}"
echo "==========================================="
