#!/usr/bin/env bash
#
# RLTR-2Stage Baseline: Qwen2.5-3B (Planner Training)
#
# ================================================================================
# RLTR-2Stage in a nutshell
# ================================================================================
# Mirrors the pipeline split described in the RLTR paper:
# - Planner (trained here): emits the tool trajectory only, rewarded on tool completeness only
# - Summariser (frozen SFT): consumes the planner trajectory and writes the final answer
#
# Key design points:
# 1. RL updates the planner only (tool tokens); summary tokens get advantage = 0
# 2. Reward = R_comp + R_repeat + R_error (no LLM judge)
# 3. Malformed output scores -1 (hard penalty)
#
# Differences from SLCA:
# - SLCA optimises tool and summary segments inside a single model
# - RLTR is a two-model pipeline: an RL-trained planner plus a frozen summariser
#
...
# Model path (3B): planner SFT checkpoint (summary stripped, <answer> tags added)
MODEL_PATH="${MODEL_PATH:-${REPO_ROOT}/outputs/planner_sft/qwen2_5_3b_planner}"

# Output directory
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/rl/qwen2_5_3b_rltr_2stage}"

# Reward function: RLTR-specific (no LLM judge)
...
# Penalty coefficient for repeated tool calls
export RLTR_LAMBDA_REPEAT="${RLTR_LAMBDA_REPEAT:-0.1}"
# Penalty coefficient for erroneous tool calls
export RLTR_MU_ERROR="${RLTR_MU_ERROR:-0.2}"

# ============================================================================
# Weights: process_score sub-weights, identical to SLCA for a fair comparison
# ============================================================================
...
# ============================================================================
# RLTR needs no LLM judge; it only scores tool-calling quality
export LLM_JUDGE_ENABLED=false
export NO_CALL_PENALTY="${NO_CALL_PENALTY:--0.5}"
export ENABLE_WANDB_LOGGING=true

echo "=========================================="
echo "RLTR-2Stage baseline: planner training (Qwen2.5-3B)"
echo "=========================================="
echo ""
echo "RLTR-2Stage key points:"
echo "  - algorithm: rltr_planner (updates tool tokens only)"
echo "  - Loss:  vanilla (plain token-mean)"
echo "  - Reward: R_comp + R_repeat + R_error"
echo "  - LLM judge: DISABLED (the planner does not need it)"
echo ""
echo "RLTR configuration:"
echo "  RLTR_COMP_CHECKER_BASE_URL: ${RLTR_COMP_CHECKER_BASE_URL}"
echo "  RLTR_COMP_CHECKER_MODEL:    ${RLTR_COMP_CHECKER_MODEL}"
echo "  RLTR_COMP_CHECKER_N:        ${RLTR_COMP_CHECKER_N}"
echo "  RLTR_COMP_CHECKER_TEMPERATURE: ${RLTR_COMP_CHECKER_TEMPERATURE}"
...
PROJECT_NAME="Toucan_baseline_rltr_qwen2.5-3B-New"
EXPERIMENT_NAME="qwen2.5_3b_rltr_2stage-New"
N_GPUS_PER_NODE=8
NNODES="${NNODES:-4}"
...
if [ "${TEST_FREQ}" != "-1" ]; then
    if [ ! -f "${VAL_DATA}" ]; then
        echo "Warning: validation file not found: ${VAL_DATA}; disabling online validation."
        TEST_FREQ=-1
    fi
fi
...
echo ""
echo "=========================================="
echo "Planner training finished."
echo "output dir : ${OUTPUT_DIR}"
echo "=========================================="
