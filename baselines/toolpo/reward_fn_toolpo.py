"""
ToolPO baseline reward function (Toucan adaptation).

Implements the R_succ + R_action reward of the ToolPO paper on Toucan
tool-calling data, and is consumed by `verl`'s `toolpo_grpo` advantage
estimator:
...
- `R_succ` is binary. ToolPO's original R_succ is a rule-based match of the
  final answer against a ground-truth answer; Toucan carries no ground-truth
  answer (only `gold_tool_calls`), so we substitute the Solved / Unsolved LLM
  judge from the DeepAgent repository, whose prompt is copied verbatim from
  `DeepAgent/src/prompts/prompts_tooleval.py`. Solved -> 1.0,
  Unsolved / Unsure -> 0.0; an initial `Unsure` falls back to the
  PARSE_ANSWER_STATUS prompt.
- `C(a_t^call)` is binary tool correctness under a strict match (exact tool
  name + every gold key present + every value loosely equal), assigned by a
  greedy one-to-one search over the predicted calls.
- `R_action = sum(C) / |gold_flat|`, normalised to [0, 1].
- `S_pref = 0`: Toucan has no memory-fold action, so the lambda_2 * S_pref term
  of the paper vanishes.

The full set of SLCA evaluation metrics (process_score, summary_score,
success@k, ...) and the Toucan response-quality judge are also computed, so
this baseline can be scored against SLCA-GRPO under one protocol. Those extra
metrics are logging-only and never enter the RL objective.

This is not a bit-exact reproduction of the original ToolPO system: the R_succ
judge, the matcher for C, the required-tool omission guard, K = 16 and the
simulator are all Toucan adaptations.
"""
...
# Prompt copied verbatim from DeepAgent/src/prompts/prompts_tooleval.py:4
...
# Prompt copied verbatim from DeepAgent/src/prompts/prompts_tooleval.py:22
    ...
    for attempt in range(LLM_JUDGE_MAX_RETRIES + 1):
        try:
                ...
                async with session.post(url, headers=headers, json=payload) as resp:
                    ...
                    if "choices" not in data or len(data["choices"]) == 0:
                        logger.warning(
                            ...
                            f"data_keys={list(data.keys())}, data={str(data)[:300]}"
                        )
                        return None
            ...
            logger.warning(
                ...
                f"{type(e).__name__}: {str(e)[:200]}, url={url}"
            )
            if attempt < LLM_JUDGE_MAX_RETRIES:
                await asyncio.sleep(LLM_JUDGE_RETRY_DELAY * (attempt + 1))
    logger.warning(f"[LLM Judge] all {LLM_JUDGE_MAX_RETRIES+1} attempts failed, prompt={prompt_preview}")
    return None
...
def _parse_rsucc_result(llm_response: str) -> str:
    ...
    Search the output for the "Solved", "Unsolved", "Unsure" keywords.
    Follows the parsing logic of DeepAgent/src/evaluate/evaluate_toolbench.py.
    """
    if not llm_response:
        return "Unsure"
    text = llm_response.strip().lower()
    # Search by priority: explicit Solved/Unsolved first, then Unsure
    ...
    if "unsure" in text:
        return "Unsure"
    # Fallback: if no keyword is found, check the first line
...
async def _compute_toolpo_rsucc(
    ...
    # Unsure → fallback: PARSE_ANSWER_STATUS_PROMPT
...
def _build_error_result(score: float, error_type: str) -> Dict[str, Any]:
    ...
    return {
        ...
        # ToolPO-specific fields
        "reward/tool_call/toolpo_r_succ": 0.0,
        "reward/tool_call/toolpo_correct_ratio": 0.0,
        "reward/tool_call/toolpo_correct_count": 0,
        "reward/tool_call/toolpo_rsucc_judge_result": "error",
    }
...
async def compute_score(
    ...
    **kwargs,
    ...
    - toolpo_correct_ratio = Σ C(a_t^call) / len(gold_flat), used for A_tool
    ...
    if gold_flat and format_result.open_count == 0:
        ...
        return {
            ...
            "reward/tool_call/toolpo_correct_ratio": 0.0,
            "reward/tool_call/toolpo_correct_count": 0,
            "reward/tool_call/toolpo_rsucc_judge_result": "Unsolved",
        }
    ...
    r_succ, rsucc_judge_result = await _compute_toolpo_rsucc(
        question_content, final_response, tool_interactions
    )
    logger.info(f"[ComputeScore] ★ ToolPO R_succ={r_succ:.0f} (judge={rsucc_judge_result})")
    ...
    logger.info(f"[ComputeScore] ★ score(R_succ)={r_succ:.0f}, format={format_score:.3f}, name={name_match_score:.3f}, key={key_match_score:.3f}, value={value_match_score:.3f}, parallel={parallel_score:.3f}, respq={response_quality_score:.3f}")
    ...
    return {
        ...
        # ToolPO-specific fields (read by the advantage estimator)
        "reward/tool_call/toolpo_r_succ": float(r_succ),
        "reward/tool_call/toolpo_correct_ratio": float(toolpo_correct_ratio),
        "reward/tool_call/toolpo_correct_count": int(toolpo_correct_count),
        "reward/tool_call/toolpo_rsucc_judge_result": rsucc_judge_result,
    }
...
def test_reward_function():
    ...
    def print_result(name, result):
        ...
        print(f"  -- SLCA evaluation metrics --")
