...
class TaskRunner:
    ...
    def init_resource_pool_mgr(self, config):
        ...
        # Heterogeneous clusters: the GPUS_PER_NODE_LIST env var gives the GPU count per node.
        # For example, GPUS_PER_NODE_LIST="4,4,6,6,6,6" means 2 nodes with 4 GPUs and 4 nodes with 6.
        gpus_per_node_list_str = os.environ.get("GPUS_PER_NODE_LIST", None)
        if gpus_per_node_list_str:
            gpus_per_node_list = [int(x.strip()) for x in gpus_per_node_list_str.split(",")]
            print(f"[verl] Using heterogeneous cluster config: GPUS_PER_NODE_LIST={gpus_per_node_list}")
            resource_pool_spec = {
                global_pool_id: gpus_per_node_list,
            }
        else:
            # Homogeneous cluster: fall back to the standard config fields.
            resource_pool_spec = {
