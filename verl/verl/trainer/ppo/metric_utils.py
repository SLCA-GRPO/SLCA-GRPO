...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # SLCA-GRPO: per-segment advantage monitoring
    # The slca_grpo estimator writes adv_process / adv_summary into the batch
    if "adv_process" in batch.batch:
        adv_process = batch.batch["adv_process"]
        metrics["critic/advantages/process_mean"] = torch.mean(adv_process).detach().item()
        metrics["critic/advantages/process_std"] = torch.std(adv_process).detach().item()
        metrics["critic/advantages/process_max"] = torch.max(adv_process).detach().item()
        metrics["critic/advantages/process_min"] = torch.min(adv_process).detach().item()

    if "adv_summary" in batch.batch:
        adv_summary = batch.batch["adv_summary"]
        metrics["critic/advantages/summary_mean"] = torch.mean(adv_summary).detach().item()
        metrics["critic/advantages/summary_std"] = torch.std(adv_summary).detach().item()
        metrics["critic/advantages/summary_max"] = torch.max(adv_summary).detach().item()
        metrics["critic/advantages/summary_min"] = torch.min(adv_summary).detach().item()
    ...
    # SLCA reward component metrics; each key is logged only if the batch carries it.
    slca_reward_keys = [
        "reward/tool_call/format_score",
        "reward/tool_call/name_match_score",
        "reward/tool_call/key_match_score",
        "reward/tool_call/value_match_score",
        "reward/tool_call/parallel_score",
        "reward/tool_call/parallel_match",
        "reward/tool_call/response_quality_score",
        "reward/tool_call/process_score",
        "reward/tool_call/summary_score",
        "reward/tool_call/tool_match_score",
        "reward/tool_call/arg_match_score",
        # Legacy fields from an earlier reward schema; no longer emitted, kept for
        # backward compatibility with older runs.
        "reward/tool_call/first_step_f1",
        "reward/tool_call/subsequent_f1",
        # Weighted score and success@ thresholds
        "reward/tool_call/weighted_score_norm",
        "reward/tool_call/total_score",
        "reward/tool_call/success@0.7",
        "reward/tool_call/success@0.8",
        "reward/tool_call/success@0.9",
        "reward/tool_call/success@1.0",
        # tool_success@ thresholds, derived from tool_match_score
        "reward/tool_call/tool_success@0.7",
        "reward/tool_call/tool_success@0.8",
        "reward/tool_call/tool_success@0.9",
        "reward/tool_call/tool_success@1.0",
    ]
    for key in slca_reward_keys:
        if key in batch.non_tensor_batch:
            arr = batch.non_tensor_batch[key]
            if isinstance(arr, np.ndarray) and arr.size > 0:
                metrics[f"{key}/mean"] = float(np.nanmean(arr))
                metrics[f"{key}/max"] = float(np.nanmax(arr))
                metrics[f"{key}/min"] = float(np.nanmin(arr))
