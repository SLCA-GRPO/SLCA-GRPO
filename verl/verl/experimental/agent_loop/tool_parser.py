...
class HermesToolParser(ToolParser):
    ...
    Extended to support:
    - Both "arguments" and "parameters" field names for tool call arguments
    - Both single object and array format for tool calls
    """
    ...
    def _parse_single_tool_call(self, obj: dict) -> FunctionCall | None:
        """Parse a single tool call object, supporting both 'arguments' and 'parameters' fields.

        Args:
            obj: A dictionary representing a tool call, expected to have 'name' and
                 either 'arguments' or 'parameters' fields.
        ...
        # Support both "arguments" and "parameters" field names
        # This handles different tool call formats (OpenAI uses "arguments", some models use "parameters")
        ...
        return FunctionCall(name=name, arguments=args_str)
        ...
        for match in matches:
            try:
                ...
                if isinstance(parsed, list):
                    # Array format - parse each tool call in the array
                    for item in parsed:
                        fc = self._parse_single_tool_call(item)
                        if fc:
                            function_calls.append(fc)
                elif isinstance(parsed, dict):
                    # Single object format
                    fc = self._parse_single_tool_call(parsed)
                    if fc:
                        function_calls.append(fc)
