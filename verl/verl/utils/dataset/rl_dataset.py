...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Fallback: if tools_kwargs is empty but ground_truth.tool_schemas is provided,
        # build a mapping from tool name to its definition so wildcard tool simulators
        # (e.g., ToucanVLLMTool) can see the schema.
        if (not tools_kwargs) and isinstance(row_dict.get("reward_model"), dict):
            gt = row_dict["reward_model"].get("ground_truth", {}) if row_dict["reward_model"] else {}
            tool_schemas = gt.get("tool_schemas") if isinstance(gt, dict) else None
            if tool_schemas:
                # tool_schemas may be a JSON string in parquet; parse if needed
                ...
                if isinstance(tool_schemas, list):
                    ...
                    for tool in tool_schemas:
                        try:
                            func = tool["function"] if "function" in tool else tool
                            name = func.get("name")
                            if name:
                                tools_kwargs[name] = {"create_kwargs": {"tool_definition": tool}}
                        except Exception:
                            continue
