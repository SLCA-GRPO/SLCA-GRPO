...
def get_distributed_backend() -> str:
    """Return the backend string for torch.distributed.init_process_group.

    Handles the case where device is CPU to avoid duplicate device type error.
    Returns:
        backend string in format "cpu:gloo,cuda:nccl" or "gloo" for CPU-only.
    """
    device_name = get_device_name()
    if device_name == "cpu":
        # CPU-only mode, just use gloo
        return "gloo"
    else:
        # Mixed mode: cpu uses gloo, gpu/npu uses nccl/hccl
