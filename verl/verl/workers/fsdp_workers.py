...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Adaptive LR override: driver can inject lr_override via meta_info
            lr_override = data.meta_info.get("lr_override", None)
            saved_lrs = None
            if lr_override is not None:
                saved_lrs = [pg["lr"] for pg in self.actor_optimizer.param_groups]
            ...
            # Restore original LRs so scheduler state stays consistent
            if saved_lrs is not None:
                for pg, orig_lr in zip(self.actor_optimizer.param_groups, saved_lrs):
                    pg["lr"] = orig_lr
            ...
            if lr_override is not None:
                metrics["actor/lr"] = lr_override
                base_lr = self.actor_lr_scheduler.get_last_lr()[0]
                metrics["actor/lr_base"] = base_lr.item() if torch.is_tensor(base_lr) else base_lr
            else:
