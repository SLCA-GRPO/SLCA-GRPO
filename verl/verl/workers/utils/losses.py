...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    # Gradient cosine similarity measurement (tool vs summary segments)
    # When enabled, compute per-segment policy losses for later gradient analysis.
    # The segment loss tensors retain their computation graph so that
    # forward_backward_batch can call torch.autograd.grad on them.
    if getattr(config, 'compute_grad_cosine', False):
        try:
            from verl.trainer.ppo.core_algos import _build_two_segment_masks
            response_mask_float = response_mask.float()
            process_mask, summary_mask, process_present, summary_present = _build_two_segment_masks(response_mask_float)

            # Tool-segment loss (process tokens only)
            tool_mask = response_mask & process_mask
            if tool_mask.any():
                tool_pg_loss, _ = policy_loss_fn(
                    old_log_prob=old_log_prob, log_prob=log_prob, advantages=advantages,
                    response_mask=tool_mask, loss_agg_mode=loss_agg_mode,
                    config=config, rollout_is_weights=rollout_is_weights,
                )
                metrics["_gc_tool_loss"] = tool_pg_loss  # NOT detached — keep grad graph

            # Summary-segment loss (summary tokens only)
            sum_mask = response_mask & summary_mask
            if sum_mask.any():
                sum_pg_loss, _ = policy_loss_fn(
                    old_log_prob=old_log_prob, log_prob=log_prob, advantages=advantages,
                    response_mask=sum_mask, loss_agg_mode=loss_agg_mode,
                    config=config, rollout_is_weights=rollout_is_weights,
                )
                metrics["_gc_sum_loss"] = sum_pg_loss  # NOT detached — keep grad graph
        except Exception:
            pass  # Silently skip if segment masks fail — don't break training
