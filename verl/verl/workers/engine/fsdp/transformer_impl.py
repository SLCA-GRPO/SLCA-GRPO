    ...
    def forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False) -> list[TensorDict]:
        ...
        for micro_batch in micro_batches:
            with ctx:
                ...
                if not forward_only:
                    # Gradient cosine similarity: cos(∇_tool, ∇_sum)
                    # Measures cross-segment gradient interference before backward.
                    # Under FSDP, each rank holds a parameter shard. We compute local
                    # dot-product and squared norms, then all_reduce(SUM) across the
                    # data-parallel group to obtain the *global* cosine similarity:
                    #   cos = Σ_i dot_i / (√(Σ_i ||t_i||²) · √(Σ_i ||s_i||²))
                    mb_metrics = meta_info.get("metrics", {})
                    gc_tool = mb_metrics.pop("_gc_tool_loss", None)
                    gc_sum = mb_metrics.pop("_gc_sum_loss", None)
                    if gc_tool is not None and gc_sum is not None:
                        try:
                            params = [p for p in self.module.parameters() if p.requires_grad]
                            grad_tool = torch.autograd.grad(gc_tool, params, retain_graph=True, allow_unused=True)
                            grad_sum = torch.autograd.grad(gc_sum, params, retain_graph=True, allow_unused=True)
                            # Flatten local shard gradients
                            gt_parts = [g.reshape(-1) for g in grad_tool if g is not None]
                            gs_parts = [g.reshape(-1) for g in grad_sum if g is not None]
                            if gt_parts and gs_parts:
                                gt_flat = torch.cat(gt_parts)
                                gs_flat = torch.cat(gs_parts)
                                # Local statistics (on this rank's parameter shard)
                                local_dot = (gt_flat * gs_flat).sum()
                                local_norm_t_sq = gt_flat.pow(2).sum()
                                local_norm_s_sq = gs_flat.pow(2).sum()
                                # All-reduce across DP group → global statistics
                                dp_group = self.get_data_parallel_group()
                                if torch.distributed.is_initialized():
                                    torch.distributed.all_reduce(local_dot, op=torch.distributed.ReduceOp.SUM, group=dp_group)
                                    torch.distributed.all_reduce(local_norm_t_sq, op=torch.distributed.ReduceOp.SUM, group=dp_group)
                                    torch.distributed.all_reduce(local_norm_s_sq, op=torch.distributed.ReduceOp.SUM, group=dp_group)
                                # Global cosine similarity
                                cos_sim = local_dot / (local_norm_t_sq.sqrt() * local_norm_s_sq.sqrt() + 1e-8)
                                mb_metrics["grad_cosine_sim"] = cos_sim.item()
                                mb_metrics["grad_norm_tool"] = local_norm_t_sq.sqrt().item()
                                mb_metrics["grad_norm_sum"] = local_norm_s_sq.sqrt().item()
                        except Exception:
                            pass  # Don't break training if gradient cosine fails
