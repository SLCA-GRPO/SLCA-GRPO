...
class MultiTurnConfig(BaseConfig):
    ...
    # === NEW: Disable tool schema injection into chat template ===
    # When True, apply_chat_template will NOT inject tool schemas automatically.
    # Tool definitions should be provided directly in the training data's instruction/system prompt.
    disable_tool_schema_injection: bool = False

    # === NEW: Wildcard tool routing ===
    # When set, ALL tool calls (regardless of tool name) will be routed to this single registered tool.
    # This enables simulation of arbitrary APIs using a single LLM-based tool simulator.
    wildcard_tool_name: Optional[str] = None
