#!/usr/bin/env bash
#
# Gradient cosine-similarity probe -- standard GRPO
# Default weights: format=0.10, name=0.25, key=0.15, value=0.20, parallel=0.30
# Enables compute_grad_cosine to log the cos(grad_tool, grad_summary) curve
# Control arm: grpo instead of slca_grpo
# data.seed=44 is pinned below purely for reproducibility; it does not identify
...
# Output directory: gradient cosine probe, GRPO (3B)
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/analysis/grad_cosine/grpo_3b}"

# Reward function shipped in this directory (default weights)
REWARD_FN_PATH="${REWARD_FN_PATH:-${REPO_ROOT}/analysis/hierr_sweep/reward_fn_hierr_v82.py}"

# ============================================================================
# HierR weights -- Default (as reported in the paper), used to compute the HierR reward
# ============================================================================
...
echo "=========================================="
echo "Gradient cosine-similarity probe -- standard GRPO (control arm)"
echo "data.seed=44 (reproducibility parameter)"
echo "compute_grad_cosine=true"
echo "=========================================="
echo ""
echo "Weights (Default, used to compute the HierR reward):"
...
PROJECT_NAME="Toucan_tool_call_slca_grpo_qwen2.5-3B_cosine"
EXPERIMENT_NAME="grad_cosine_grpo_3b"
N_GPUS_PER_NODE=8
NNODES="${NNODES:-4}"
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.compute_grad_cosine=true \
    \
